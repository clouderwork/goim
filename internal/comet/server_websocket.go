...
func (s *Server) ServeWebsocket(conn net.Conn, rp, wp *bytes.Pool, tr *xtime.Timer) {
	...
	if p, err = ch.CliProto.Set(); err == nil {
		if ch.Mid, ch.Key, ch.Platform, rid, accepts, hb, err = s.authWebsocket(ctx, ws, p, req.Header.Get("Cookie")); err == nil {
			ch.Watch(accepts...)
			b = s.Bucket(ch.Key)
			err = b.Put(rid, ch)
			if conf.Conf.Debug {
...
func (s *Server) authWebsocket(ctx context.Context, ws *websocket.Conn, p *grpc.Proto, cookie string) (mid logicapi.MidType, key string, platform string, rid string, accepts []int32, hb time.Duration, err error) {
	for {
		if err = p.ReadWebsocket(ws); err != nil {
			return
		}
	...
	if mid, key, platform, rid, accepts, hb, err = s.Connect(ctx, p, cookie); err != nil {
		return
	}
